...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        #if person haven't parents: take unconditional probs
        if not mother:
            ...
            mom = gene_pass(mother)
            dad = gene_pass(father)
            if genes == 2:  
                #mom and dad pass one copy of the gene
                gene_prob = mom * dad
            elif genes == 1:
                #either mom or dad pass one copy 
                gene_prob = mom * (1 - dad) + (1 - mom) * dad
            else:
                #neither of them pass
                gene_prob = (1 - mom) * (1 - dad)
        
        trait_prob = PROBS["trait"][genes][trait]
        joint_proba *= gene_prob * trait_prob
